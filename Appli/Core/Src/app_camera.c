...
static void app_camera_init_nn_crop_config(CMW_Manual_roi_area_t *roi, uint32_t sensor_width, uint32_t sensor_height);
...
/* 保存 LCD 和 NN 管道的 ROI 信息, 用于坐标映射 */
static CMW_Manual_roi_area_t lcd_roi;
static CMW_Manual_roi_area_t nn_roi;
...
static void app_camera_display_pipe_init(uint32_t sensor_width, uint32_t sensor_height)
{
    ...
    lcd_roi = cmw_dcmipp_conf.manual_conf;
...
static void app_camera_nn_pipe_init(uint32_t sensor_width, uint32_t sensor_height)
{
    ...
    cmw_dcmipp_conf.enable_gamma_conversion = 0;
    cmw_dcmipp_conf.mode = CMW_Aspect_ratio_manual_roi;
    app_camera_init_nn_crop_config(&cmw_dcmipp_conf.manual_conf, sensor_width, sensor_height);
    nn_roi = cmw_dcmipp_conf.manual_conf;
...
/* NN 管道使用独立正方形 ROI, 取传感器短边居中裁剪, 避免非等比例缩放 */
static void app_camera_init_nn_crop_config(CMW_Manual_roi_area_t *roi, uint32_t sensor_width, uint32_t sensor_height)
{
    uint32_t side;

    side = MIN(sensor_width, sensor_height);

    roi->width = side;
    roi->height = side;
    roi->offset_x = (sensor_width - side) / 2U;
    roi->offset_y = (sensor_height - side) / 2U;
}
...
void app_camera_get_lcd_roi(CMW_Manual_roi_area_t *roi)
{
    *roi = lcd_roi;
}

void app_camera_get_nn_roi(CMW_Manual_roi_area_t *roi)
{
    *roi = nn_roi;
}
